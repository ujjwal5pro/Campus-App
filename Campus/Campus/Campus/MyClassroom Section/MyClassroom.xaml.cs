...
{
    public partial class MyClassroom : ContentPage
    {
        ...
        private async void Onnotificaitonclicked(object sender, EventArgs e)
        {
            await RefreshItems(true, syncItems: true);
        }
        ...
        {
            string Dpartment = Application.Current.Properties["Department"] as string;
            using (var scope = new ActivityIndicatorScope(syncIndicator, showActivityIndicator))
            {
                var table = await manager.GetTodoItemsAsync(syncItems);
                todoList.ItemsSource = table.Where(student =>student.Department==Dpartment);
            }
